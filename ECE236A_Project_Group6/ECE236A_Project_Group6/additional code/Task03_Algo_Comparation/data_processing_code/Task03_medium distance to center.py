...
class MyLabelSelection:
    ...
    def select(self, trainX):
        ...
        sum_dis = 0  # initialize the sum of distance
        for i in range(d):
            sum_dis += distance[i]
        mean_dis = [sum_dis / d for _ in range(d)]
        dis_mid = [x - y for x, y in zip(distance, mean_dis)]  # 到中间值的距离
        dis_mid = [abs(x) for x in dis_mid]
        data_to_label = np.argsort(dis_mid)[d-k:d]
